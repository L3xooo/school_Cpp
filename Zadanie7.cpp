...
queue<int> reverse(const list<int> & data) noexcept {
    ...
    return stack; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    std::vector<vector<int>> Vector(size1);
    for(size_t a = 0; a < size1 ; a++){
        for(size_t b = 0; b < size2;b++){
            Vector.at(a).push_back(value);
        }
    }  
...
string connectWithSpace(const string & text, const int number) noexcept {
    // TODO
    string newWord;
    newWord = text+" "+to_string(number);
    cout << newWord;
    return newWord; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    ...
    for(string word:sentence){
        if(auto search = translator.find(word);search != translator.end())
            translatedList.push_back(search->second);
        else
            translatedList.push_back("?");
    }
    return translatedList; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    size_t length = mathExpression.length();
    std::stack<char> Stack;
    bool result = true;
    for(size_t a = 0; a < length ; a++){
        if(mathExpression[a] == '(' || mathExpression[a] == '{' 
        || mathExpression[a] == '[' || mathExpression[a] == '<'){
            Stack.push(mathExpression[a]);
            //cout << Stack.top() << " Saved to stack" <<endl;
        }
        else if(mathExpression[a] == ')' || mathExpression[a] == '}' 
        || mathExpression[a] == ']' || mathExpression[a] == '>'){
            //cout << "Inside" << endl;
            if(Stack.empty() == true){
                //cout << "Stack is empty";
            ...
            if(mathExpression[a] == ')' && (char)(mathExpression[a]-1) == Stack.top()){
               // cout << "Comparing " << mathExpression[a] << " " <<Stack.top() << "Pop from stack" << endl;
                Stack.pop();
            }
            else if((char)(mathExpression[a]-2) == Stack.top()){
               // cout << "Comparing " << mathExpression[a] << " " <<Stack.top() << "Pop from stack" << endl;
                Stack.pop();
            }
            else{
...
int execute(int (*function)(int, int), int parameter1, int parameter2) {
    int result = function(parameter1,parameter2);
    cout << result ;
    return result; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int> createList(const list<int>::size_type length, const list<int>::size_type indexOfOne) {
    list<int> createdList;
    if(indexOfOne >= length)
        throw IndexOutOfRangeException(length,indexOfOne);
    for(size_t a = 0; a < length ; a++){
        if(a == indexOfOne)
            createdList.push_back(1);
        else
            createdList.push_back(0);
    }
    return createdList; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int main() {
    ...
    //Uloha5
        ...
        //     cout << (sum(iss)); 
    //Uloha6
        // std::map<string,string> translator = {{"read","citat"},{"write","pisat"},
        // {"book","kniha"},{"I","ja"},{"you","ty"},{"he","on"},
        // {"she","ona"},{"it","ono"},{"a",""},{"an",""},{"the",""}};
        // list<string> sentence = { "I", "read", "a", "book"};
        // translate(translator,sentence);
    // tu mozete doplnit testovaci kod
    //Uloha7
        auto start = high_resolution_clock::now();
        areParenthesisPaired("");
    ...
    //Uloha8
        //execute([](int a, int b)->int { return a - b; }, 20, 50);
    //Uloha90
